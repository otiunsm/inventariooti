...
<div class="p-6 bg-white rounded-lg shadow-md">
    <h3 class="text-xl font-bold mt-8 mb-4 text-green-700"><?= esc($titulo) ?></h3>
    ...
    <div id="modalCrearUnidad" class="hidden fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
        <div class="bg-white p-8 rounded-lg shadow-xl w-96 max-w-full">
            ...
            <form action="<?= base_url('Catalogo/UnidadOrganicaAcadem/crear') ?>" method="post" class="space-y-4">
                <?= csrf_field() ?>
        ...
        <div class="rounded-lg shadow-lg border border overflow-hidden">
            <table class="w-full rounded-lg shadow-md border border-green-300">
                <thead class="bg-green-700 text-white">
                    <tr>
                        <th>Nombre Unidad</th><th>Sede</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-green-200">
                    <?php if (!empty($unidad_organica)): ?>
                        <?php foreach ($unidad_organica as $i => $unidad): ?>
                                ...
                                <td class="px-5 py-3 flex justify-center space-x-2">
                                    ...
                                    <div id="modalEditarUnidad" class="hidden fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                                        <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
                                            ...
                                            <!-- Formulario -->
                                            <form id="formEditarUnidad" action="<?= base_url('Catalogo/UnidadOrganicaAcadem/editar') ?>" method="post">
                                                <?= csrf_field() ?>
                                    ...
                                    <a href="<?= base_url('DetalleUnidad/'.$unidad['id_unidad_organica'])?>" 
                                        class="text-blue-600 hover:text-blue-800" title="Ver detalle" aria-label="Ver detalles">
                                    ...
                                    <!-- Botón Eliminar -->
                                    <form action="<?= base_url('Catalogo/UnidadOrganicaAcadem/eliminar/'.$unidad['id_unidad_organica']) ?>" method="post" 
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="3" class="px-4 py-4 text-center text-gray-500">No hay unidades registradas</td>
                        </tr>
                    <?php endif ?>
                </tbody>
            </table>
